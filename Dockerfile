...
RUN dotnet publish "CertificateUsage.Listener/CertificateUsage.Listener.fsproj" -c Release -o /app -r alpine-x64 --self-contained false --no-restore
...
WORKDIR /app
COPY --from=build /app .
COPY --from=build /source/health.sh .
ENV DOTNET_SYSTEM_GLOBALIZATION_INVARIANT=1 \
  DOTNET_RUNNING_IN_CONTAINER=true

CMD ["dotnet", "CertificateUsage.Listener.dll"]
